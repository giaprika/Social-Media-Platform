...
export const addMessage =
  ...
  async (dispatch) => {
    ...
    try {
      await postDataAPI("/messages/message", msg);
    } catch (err) {
      dispatch({
...
export const getConversations =
  ...
  async (dispatch) => {
    try {
      const res = await getDataAPI(`/messages/conversations?limit=${page * 9}`);
      let newArr = [];
...
export const getMessages =
  ...
  async (dispatch) => {
    try {
      const res = await getDataAPI(`/messages/message/${id}?limit=${page * 9}`);
