...
// connect but don't crash if Redis unavailable
...
const set = async (
  key,
  value,
  ttl = parseInt(process.env.CACHE_DEFAULT_TTL || "60", 10)
) => {
  try {
...
const del = async (patternOrKey) => {
  try {
    ...
    const keys = [];
    for await (const k of client.scanIterator({ MATCH: patternOrKey }))
      keys.push(k);
