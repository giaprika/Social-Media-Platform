...
const adminCtrl = {
  ...
  getTotalLikes: async (req, res) => {
    try {
      const posts = await Posts.find();
      let total_likes = 0;
      posts.map((post) => (total_likes += post.likes.length));
  ...
  getSpamPosts: async (req, res) => {
    try {
      const posts = await Posts.find().select("user createdAt reports content").populate({ path: "user", select: "username avatar email" });
