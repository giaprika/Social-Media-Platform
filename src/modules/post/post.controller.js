...
const postCtrl = {
  createPost: async (req, res) => {
    try {
      ...
      if (images.length === 0) return res.status(400).json({ msg: "Please add photo(s)" });
  ...
  updatePost: async (req, res) => {
    try {
      const { content, images } = req.body;
      const post = await Posts.findOneAndUpdate({ _id: req.params.id, user: req.user._id }, { content, images })
        .populate("user likes", "avatar username fullname")
        .populate({ path: "comments", populate: { path: "user likes ", select: "-password" } });

      if (!post) return res.status(400).json({ msg: "Post does not exist or you are not the owner." });
  ...
  deletePost: async (req, res) => {
    try {
      ...
      res.json({ msg: "Post deleted successfully.", newPost: { ...post, user: req.user } });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },
  ...
  reportPost: async (req, res) => {
    try {
      const post = await Posts.find({ _id: req.params.id, reports: req.user._id });
      if (post.length > 0) return res.status(400).json({ msg: "You have already reported this post" });

      const report = await Posts.findOneAndUpdate({ _id: req.params.id }, { $push: { reports: req.user._id } }, { new: true });
  ...
  savePost: async (req, res) => {
    try {
      ...
      if (user.length > 0) return res.status(400).json({ msg: "You have already saved this post." });
