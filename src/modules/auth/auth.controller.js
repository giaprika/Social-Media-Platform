...
const authCtrl = {
  register: async (req, res) => {
    try {
      ...
      res.json({ msg: "Registered Successfully!", access_token, user: { ...newUser._doc, password: "" } });
  ...
  registerAdmin: async (req, res) => {
    try {
      ...
      const passwordHash = await bcrypt.hash(password, 12);
      const newUser = new Users({ fullname, username: newUserName, email, password: passwordHash, gender, role });
      await newUser.save();
  ...
  login: async (req, res) => {
    try {
      ...
      const user = await Users.findOne({ email, role: "user" }).populate("followers following", "-password");
      ...
      res.json({ msg: "Logged in  Successfully!", access_token, user: { ...user._doc, password: "" } });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
  },
  ...
  adminLogin: async (req, res) => {
    try {
      ...
      res.cookie("refreshtoken", refresh_token, { httpOnly: true, path: "/api/refresh_token", maxAge: 30 * 24 * 60 * 60 * 1000 });
      res.json({ msg: "Logged in  Successfully!", access_token, user: { ...user._doc, password: "" } });
    } catch (err) {
      return res.status(500).json({ msg: err.message });
    }
